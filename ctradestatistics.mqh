...
public:
   ...
   void              InitRiskFreeRate(double rfr);
  };
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
void CTradeStatistics::InitRiskFreeRate(double rfr)
  {
...
double CTradeStatistics::Percent(double value,double divider)
  {
   if(MathAbs(value)<=FLT_EPSILON)
      return(0);
   return(MathAbs(divider)>FLT_EPSILON?100*value/divider:0);
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
double CTradeStatistics::Divide(double value,double divider)
  {
   if(MathAbs(value)<=FLT_EPSILON)
      return(0);
   return(MathAbs(divider)>FLT_EPSILON?value/divider:0);
  }
//+------------------------------------------------------------------+
//|   Calculate()                                                    |
//+------------------------------------------------------------------+
         ...
         if(profit>=0.0)
           {
            ...
            if(initial_deposit==0.0)
               if(m_initial_deposit==0.0)
                  m_initial_deposit=m_deal.Profit();// set initial deposit
           }
         //--- withdrawal
         else
      ...
      for(int i=1; i<limit; i++)
        {
         if(m_balance_data.At(i)!=0.0)
           {
            HPR[i-1]=MathAbs(m_balance_data.At(i-1))>0?m_balance_data.At(i)/m_balance_data.At(i-1):1;
            m_ahpr+=HPR[i-1];
           }
        }
      ...
      if(limit>2)
        {
         ...
         m_sharpe_ratio=(m_ahpr-(1.0+this.RiskFreeRate))/(Std>0?Std:1);
        }
     }
...
// PrintFormat("cov = %.2f Sx=%.2f Sy=%.2f",cov,Sx,Sy);
   Correlation=MathAbs(Sx*Sy)>0?sign*cov/(Sx*Sy):0;
